...
Muc tieu:
- Hieu vi sao RTSP thuong dung `uridecodebin` / source bin.
- Hieu su khac nhau giua file source va live source.
- Hieu vi sao `streammux.set_property("live-source", True)` quan trong.

Cach chay:
    python3 exercises/09_rtsp_source_walkthrough.py rtsp://user:pass@host:554/path
"""
...
def main(args):
    ...
    # TODO: Them `latency` cho `rtspsrc` neu can truy cap sau hon qua source bin.
    # TODO: Doi `fakesink` thanh sink hien thi neu muon xem live output.
    # TODO: Thu bo `live-source=True` va quan sat van de timestamp / batching.
    #
    # SELF-CHECK:
    # - Tai sao bai RTSP khong con dung `filesrc -> h264parse` truc tiep?
    # - Ghost pad cua source bin giai quyet bai toan gi?
    return 0
